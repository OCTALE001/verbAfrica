...
**Batch B — typography + skeletons + ratings to 0 (committed):** Added Google Fonts (Inter body, Space Grotesk for the wordmark / hero / names / section titles) with display=swap and preconnect; tokenised as --font-body / --font-head. Added loading skeleton cards (8 shimmer placeholders) shown via a dataLoaded flag while creatives.json is in flight, replaced by real cards on resolve. Dropped the fabricated ratings: fakeRating/fakeReviews now return 0 and the card hides the star row, showing "New to verbAfrica" instead of invented stars — reversible when real reviews exist. Verified in preview: Inter+Space Grotesk load, 11 cards render, 8 skeletons while loading, no stars, no console errors. (Note: preview_screenshot returns black on the long scrolled browse page due to smooth-scroll capture timing — the landing screenshot and DOM checks confirm the page is healthy.)

**Batch C — emoji to SVG icons (committed):** Replaced every colour emoji used as a UI icon with inline Lucide-style SVG. Built a small icon system: an ICONS map (music/headphones/mic/camera/film/phone_device/search/briefcase/clipboard/instagram/message/heart/compare/x/check/mail/phone/target/eye/wallet/star/edit/clock/doc), an icon(name,cls) helper that emits a hydratable placeholder, and hydrateIcons(root) that fills .ico[data-ico] placeholders idempotently. Static HTML uses placeholders hydrated on load; dynamic templates emit icon() and call hydrateIcons after each render (grid, detail, shortlist, brief matches, cp profile/epk/enquiries). Also caught and fixed stragglers the first grep missed (👁 💰 ⏱ 📨 ✏️ 📌 📄 🎉) and dropped emoji from toast strings. Monochrome typographic glyphs (✕ ✓ ★ ◎ ● → ✦) left as-is — they render as clean text, not colour emoji. Verified: all icons hydrate (0 unhydrated placeholders), no console errors. Aligns with Alexander's no-emoji preference.

**Code-review cleanup (committed):** Cleared the low-severity items from the earlier review. Hardened igUrl with an igHandle() normaliser (strips whitespace, leading @s, a pasted instagram.com URL, trailing path, invalid chars) and extracted a single igLink() helper used by all three Instagram link sites (card / detail / EPK) instead of three hand-rolled anchors. Cached the search index per creative (c._sidx / c._swords) so searchMatch no longer rebuilds + lowercases + regex-splits on every keystroke. Removed dead code: the always-true state.compareMode flag, the orphaned .compare-toggle-hint CSS, and the duplicated background-size:cover avatar rule near the top (kept the effective one at the end, with a comment explaining the source-order requirement). Verified: fuzzy search, IG sanitisation, and compare flow all still work; no console errors.

**DEPLOYED 2026-06-24:** Pushed 5 commits (b5973e7 → f7a7e57) to origin/main — live on octale001.github.io. First deploy of the feedback-round-1 work: purple/blue rebrand, all 12 WhatsApp fixes, green CTAs, accessibility/touch pass, typography + skeletons, ratings dropped to 0, emoji→SVG icons, and the code-review cleanup. The Boys of Soweto jersey photo (Alexander sent it in chat) is deferred to the next round — it's a full-length editorial shot that won't crop well to the 1.5:1 face-forward cards; needs a tighter chest-up crop or imagePos tuning. Untracked local files (.DS_Store, .claude/, 6 loose source images, memory) intentionally not pushed.

**2026-07-02 — First cohort feedback lands:** Shivolski reviewed his profile and sent two copy corrections via Alexander: "Dance collaborations with 250 Machine" becomes "Dance collaboration with various influencers", and "Mr Bully Tour" joins his festivals highlight. Small edits, but a milestone — the cohort is now actively shaping their own profiles. Applied in data/creatives.json and pushed. The stale "student events" wording in the same highlight was flagged again; Alexander chose to leave it for now.

**2026-07-02 — First site audit:** Full pass over every page after the Shivolski push: no console errors, no failed requests, all 11 photos live, mobile clean. Findings were all copy: the landing page still promises equipment hire, technicians and "ratings and reviews" (none exist yet — flagged for later with "track bookings and earnings"), and How It Works claimed "15+ creatives" when the roster is 11 — fixed to "a growing roster". One design miss from the a11y pass got fixed too: the two landing CTAs were 17px-tall text links; now proper 44px touch targets with the same look.

**2026-07-05 — Chef Nqobi's profile, in his own words:** Second cohort member to send feedback. Full rewrite of his About (real name Nqobile Kwame Nkabinde, creative-director positioning, luxury hospitality framing), new highlights (Decorex Joburg 2025, Samsung/Red Bull/Mr D brand collabs, ANEW internship, Borwa Design Week 2024) and notable work (five ANEW hotels + Marriott Loftus, Melodies and Munchies, TastyTing, Taste The Beat). Alexander previewed and fine-tuned line placement twice before approving — the review-preview-push rhythm is settling in. Live in 3047ec8.

**2026-07-06 — Just Jabba arrives fully formed:** Third cohort feedback round, and the biggest: a complete self-authored profile — four-paragraph bio ("Artist. Builder. Organiser. Storyteller."), 12 highlights, 11 notable works led by Key Player (2025), and a whole new "Beyond the Music" section listing his 11 roles from tour manager to community builder. The site grew to fit him: bios now support paragraphs, profiles support an optional beyond-the-music section (public modal, dashboard and EPK), and along the way we found and fixed a site-wide regression — genre/location pills in every profile header had lost their styling and were rendering as plain text. Alexander previewed and approved in one pass.
